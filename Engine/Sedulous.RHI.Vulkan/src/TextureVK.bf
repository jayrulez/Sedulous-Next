...
namespace Sedulous.RHI.Vulkan
{
	...
	class TextureVK : Texture
	{
		...
		public Result Create(in TextureDesc textureDesc)
		{
			...
			m_Extent = .()
				{
					width = textureDesc.size[0],
					height = textureDesc.size[1],
					depth = textureDesc.size[2]
				};
			m_MipNum = textureDesc.mipNum;
			m_ArraySize = textureDesc.arraySize;
			m_Format = textureDesc.format;
			m_SampleCount = VulkanUtils.GetSampleCount(textureDesc.sampleNum);

			readonly VkImageType imageType = VulkanUtils.GetVkImageType(textureDesc.type);
			...
			info.format = VulkanUtils.GetVkFormat(m_Format);
			...
			info.usage = VulkanUtils.GetVkImageUsageFlags(textureDesc.usageMask);
			...
			m_ImageAspectFlags = VulkanUtils.GetVkImageAspectFlags(textureDesc.format);
			...
			for (uint32 i = 0; i < m_Device.GetPhyiscalDeviceGroupSize(); i++)
			{
				...
				{
					readonly VkResult result = vkCreateImage(m_Device, &info, m_Device.GetAllocationCallbacks(), &m_Handles[i]);
		...
		public Result Create(in TextureVulkanDesc textureDesc)
		{
			...
			m_MipNum = textureDesc.mipNum;
			m_ArraySize = textureDesc.arraySize;
			m_Format = VulkanUtils.GetFormat((VkFormat)textureDesc.vkFormat);
			m_ImageAspectFlags = (VkImageAspectFlags)textureDesc.vkImageAspectFlags;
			m_TextureType = VulkanUtils.GetTextureType((VkImageType)textureDesc.vkImageType);
		...
		public void GetTextureVK(uint32 physicalDeviceIndex, ref TextureVulkanDesc textureVulkanDesc)
		{
			...
			textureVulkanDesc.vkFormat = VulkanUtils.GetVkFormat(GetFormat());
			textureVulkanDesc.vkImageAspectFlags = GetImageAspectFlags();
			textureVulkanDesc.vkImageType = VulkanUtils.GetVkImageType(GetTextureType());
		...
		public ~this()
		{
			...
			{
				if (m_Handles[i] != .Null)
					vkDestroyImage(m_Device, m_Handles[i], m_Device.GetAllocationCallbacks());
			}
		}
