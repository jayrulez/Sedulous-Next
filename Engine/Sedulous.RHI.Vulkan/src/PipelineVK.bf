...
namespace Sedulous.RHI.Vulkan
{
	public static
	{
		public static VkDynamicState[4] DYNAMIC_STATE = .(
	...
	class PipelineVK : Pipeline
	{
		...
		private Result CreateRenderPass(OutputMergerDesc* outputMerger, RasterizationDesc* rasterizationDesc)
		{
			...
			if (outputMerger != null)
			{
					...
					attachmentDescs[i] = .()
						{
							...
							format = VulkanUtils.GetVkFormat(outputMerger.color[i].format),
							samples = sampleNum,
				...
				if (hasDepthStencil)
				{
					attachmentDescs[outputMerger.colorNum] = .()
						{
							flags = (VkAttachmentDescriptionFlags)0,
							format = VulkanUtils.GetVkFormat(outputMerger.depthStencilFormat),
							samples = sampleNum,
							loadOp = .VK_ATTACHMENT_LOAD_OP_LOAD,
							storeOp = .VK_ATTACHMENT_STORE_OP_STORE,
							stencilLoadOp = .VK_ATTACHMENT_LOAD_OP_LOAD,
			...
			readonly VkResult result = vkCreateRenderPass(m_Device, &info, m_Device.GetAllocationCallbacks(), &m_RenderPass);
				...
				attributes[i] = .()
					{
						...
						format = VulkanUtils.GetVkFormat(attribute_desc.format),
				...
				attachments[i] = .()
					{
						...
						srcColorBlendFactor = VulkanUtils.GetBlendFactor(attachmentDesc.colorBlend.srcFactor),
						dstColorBlendFactor = VulkanUtils.GetBlendFactor(attachmentDesc.colorBlend.dstFactor),
						colorBlendOp = VulkanUtils.GetBlendOp(attachmentDesc.colorBlend.func),
						srcAlphaBlendFactor = VulkanUtils.GetBlendFactor(attachmentDesc.alphaBlend.srcFactor),
						dstAlphaBlendFactor = VulkanUtils.GetBlendFactor(attachmentDesc.alphaBlend.dstFactor),
						alphaBlendOp = VulkanUtils.GetBlendOp(attachmentDesc.alphaBlend.func),
						colorWriteMask = VulkanUtils.GetColorComponent(attachmentDesc.colorWriteMask)
					};
			}
		}
		...
		private Result SetupShaderStage(ref VkPipelineShaderStageCreateInfo stage, ShaderDesc shaderDesc, ref VkShaderModule* modules)
		{
			...
			readonly VkResult result = vkCreateShaderModule(m_Device, &moduleInfo, m_Device.GetAllocationCallbacks(), &module);
			...
			stage = .()
				{
					...
					stage = (VkShaderStageFlags)VulkanUtils.GetShaderStageFlags(shaderDesc.stage),
					module = module,
					pName = null,
		...
		/////////////////////////////Internal Methods//////////////////////////////
		public static implicit operator VkPipeline(Self self) => self.m_Handle;
		...
		public Result Create(GraphicsPipelineDesc graphicsPipelineDesc)
		{
			...
			FillRasterizationState(graphicsPipelineDesc, ref rasterizationState);
			...
			readonly VkResult vkResult = vkCreateGraphicsPipelines(m_Device, .Null, 1, &info, m_Device.GetAllocationCallbacks(), &m_Handle);

			RETURN_ON_FAILURE!(m_Device.GetLogger(), vkResult == .VK_SUCCESS, GetReturnCode(vkResult),
				"Can't create a graphics pipeline: vkCreateGraphicsPipelines returned {0}.", (int32)vkResult);

			for (int i = 0; i < graphicsPipelineDesc.shaderStageNum; i++)
				vkDestroyShaderModule(m_Device, modulesBegin[i], m_Device.GetAllocationCallbacks());
		...
		public Result Create(ComputePipelineDesc computePipelineDesc)
		{
			...
			VkResult result = vkCreateShaderModule(m_Device, &moduleInfo, m_Device.GetAllocationCallbacks(), &module);
			...
			result = vkCreateComputePipelines(m_Device, .Null, 1, &info, m_Device.GetAllocationCallbacks(), &m_Handle);

			RETURN_ON_FAILURE!(m_Device.GetLogger(), result == .VK_SUCCESS, GetReturnCode(result),
				"Can't create a compute pipeline: vkCreateComputePipelines returned {0}.", (int32)result);

			vkDestroyShaderModule(m_Device, module, m_Device.GetAllocationCallbacks());
		...
		public Result Create(RayTracingPipelineDesc rayTracingPipelineDesc)
		{
			...
			readonly VkResult vkResult = vkCreateRayTracingPipelinesKHR(m_Device, .Null, .Null, 1, &createInfo,
		...
		public Result CreateGraphics(VkPipeline vkPipeline)
		{
		...
		public Result CreateCompute(VkPipeline vkPipeline)
		{
			m_OwnsNativeObjects = false;
			m_Handle = vkPipeline;
			m_BindPoint = .VK_PIPELINE_BIND_POINT_COMPUTE;
		...
		public ~this()
		{
			...
			if (m_Handle != .Null)
				vkDestroyPipeline(m_Device, m_Handle, m_Device.GetAllocationCallbacks());
		...
		{
			readonly int dataSize = (int)(shaderGroupNum * m_Device.GetDesc().rayTracingShaderGroupIdentifierSize);

			readonly VkResult result = vkGetRayTracingShaderGroupHandlesKHR(m_Device, m_Handle, baseShaderGroupIndex, shaderGroupNum, (.)dataSize, buffer);
