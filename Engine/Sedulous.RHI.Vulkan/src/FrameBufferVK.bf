...
namespace Sedulous.RHI.Vulkan
{
	class FrameBufferVK : FrameBuffer
	{
		...
		private Result CreateRenderPass(FrameBufferDesc frameBufferDesc)
		{
			...
			VkResult result = vkCreateRenderPass(m_Device, &renderPassInfo, m_Device.GetAllocationCallbacks(), &m_RenderPassWithClear);
			...
			result = vkCreateRenderPass(m_Device, &renderPassInfo, m_Device.GetAllocationCallbacks(), &m_RenderPass);
		...
		public Result Create(FrameBufferDesc frameBufferDesc)
		{
			...
			for (uint32 i = 0; i < m_Device.GetPhyiscalDeviceGroupSize(); i++)
			{
				if ((1 << i) & physicalDeviceMask != 0)
				{
					...
					readonly VkResult res = vkCreateFramebuffer(m_Device, &framebufferInfo, m_Device.GetAllocationCallbacks(), &m_Handles[i]);
		...
		public VkRenderPass GetRenderPass(RenderPassBeginFlag renderPassBeginFlag)
		{
		...
		public ~this()
		{
			...
			{
				if (m_Handles[i] != .Null)
					vkDestroyFramebuffer(m_Device, m_Handles[i], m_Device.GetAllocationCallbacks());
			}
		...
		public override void SetDebugName(StringView name)
		{
			...
			m_Device.SetDebugNameToTrivialObject(.VK_OBJECT_TYPE_RENDER_PASS, (uint64)m_RenderPass.Handle, name);
			m_Device.SetDebugNameToTrivialObject(.VK_OBJECT_TYPE_RENDER_PASS, (uint64)m_RenderPassWithClear.Handle, name);
		}
	}
}