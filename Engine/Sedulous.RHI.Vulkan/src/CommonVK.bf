...
struct DeviceCreationVulkanDesc
{
	...
	public readonly VkPhysicalDevice* vkPhysicalDevices;
	public uint32 deviceGroupSize;
	public readonly uint32* queueFamilyIndices;
	...
	public readonly char8* vulkanLoaderPath;
}
...
public static class VulkanUtils
{
	...
	public const TextureType[(.)TextureType.MAX_NUM] TEXTURE_TYPE_TABLE = .(
		TextureType.TEXTURE_1D, // VK_IMAGE_TYPE_1D
		TextureType.TEXTURE_2D, // VK_IMAGE_TYPE_2D
		TextureType.TEXTURE_3D // VK_IMAGE_TYPE_3D
		);

	public const VkIndexType[(uint32)IndexType.MAX_NUM] INDEX_TYPE_TABLE = .(
		.VK_INDEX_TYPE_UINT16,
		.VK_INDEX_TYPE_UINT32
		);

	public const VkImageLayout[(uint32)TextureLayout.MAX_NUM] LAYOUT_TABLE = .(
	...
	public static VkBufferUsageFlags GetBufferUsageFlags(BufferUsageBits usageMask, uint32 structureStride)
	{
		...
		if (usageMask & BufferUsageBits.VERTEX_BUFFER != 0)
			flags |= .VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;

		if (usageMask & BufferUsageBits.INDEX_BUFFER != 0)
			flags |= .VK_BUFFER_USAGE_INDEX_BUFFER_BIT;

		if (usageMask & BufferUsageBits.CONSTANT_BUFFER != 0)
			flags |= .VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;

		if (usageMask & BufferUsageBits.ARGUMENT_BUFFER != 0)
			flags |= .VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT;

		// TODO: add more usage bits
		if (usageMask & BufferUsageBits.RAY_TRACING_BUFFER != 0)
		{
			flags |= .VK_BUFFER_USAGE_SHADER_BINDING_TABLE_BIT_KHR | .VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR |
				.VK_BUFFER_USAGE_STORAGE_BUFFER_BIT;
		}

		if (usageMask & BufferUsageBits.ACCELERATION_STRUCTURE_BUILD_READ != 0)
			flags |= .VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR;

		if (usageMask & BufferUsageBits.SHADER_RESOURCE != 0)
		{
		...
		if (usageMask & BufferUsageBits.SHADER_RESOURCE_STORAGE != 0)
		{
	...
	public static VkAccessFlags GetAccessFlags(AccessBits mask)
	{
		...
		if (mask & AccessBits.VERTEX_BUFFER != 0)
			flags |= .VK_ACCESS_VERTEX_ATTRIBUTE_READ_BIT;

		if (mask & AccessBits.INDEX_BUFFER != 0)
			flags |= .VK_ACCESS_INDEX_READ_BIT;

		if (mask & AccessBits.CONSTANT_BUFFER != 0)
			flags |= .VK_ACCESS_UNIFORM_READ_BIT;

		if (mask & AccessBits.ARGUMENT_BUFFER != 0)
			flags |= .VK_ACCESS_INDIRECT_COMMAND_READ_BIT;

		if (mask & AccessBits.SHADER_RESOURCE != 0)
			flags |= .VK_ACCESS_SHADER_READ_BIT;

		if (mask & AccessBits.SHADER_RESOURCE_STORAGE != 0)
			flags |= .VK_ACCESS_SHADER_READ_BIT | .VK_ACCESS_SHADER_WRITE_BIT;

		if (mask & AccessBits.COLOR_ATTACHMENT != 0)
			flags |= .VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT; // TODO: add READ bit too?

		if (mask & AccessBits.DEPTH_STENCIL_WRITE != 0)
			flags |= .VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;

		if (mask & AccessBits.DEPTH_STENCIL_READ != 0)
			flags |= .VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT;

		if (mask & AccessBits.COPY_SOURCE != 0)
			flags |= .VK_ACCESS_TRANSFER_READ_BIT;

		if (mask & AccessBits.COPY_DESTINATION != 0)
			flags |= .VK_ACCESS_TRANSFER_WRITE_BIT;

		if (mask & AccessBits.ACCELERATION_STRUCTURE_READ != 0)
			flags |= .VK_ACCESS_ACCELERATION_STRUCTURE_READ_BIT_KHR;

		if (mask & AccessBits.ACCELERATION_STRUCTURE_WRITE != 0)
			flags |= .VK_ACCESS_ACCELERATION_STRUCTURE_WRITE_BIT_KHR;

		return flags;
	}

	public const VkDescriptorType[(uint32)DescriptorType.MAX_NUM] DESCRIPTOR_TYPES = .(
	...
	public const VkShaderStageFlags[(uint32)ShaderStage.MAX_NUM] SHADER_STAGE_TABLE = .(
	...
	public const VkFormat[(uint32)Format.MAX_NUM] VK_FORMAT = .(
	...
	public const VkFormat[(uint32)Format.MAX_NUM] VK_IMAGE_VIEW_FORMAT = .(
		.VK_FORMAT_UNDEFINED, // UNKNOWN
	...
	public const VkImageCreateFlags[(int)Format.MAX_NUM] IMAGE_CREATE_FLAGS = .(
		0, // UNKNOWN,
	...
	{
		return IMAGE_CREATE_FLAGS[(int)format];
	}
	...
	public const Format[261] FORMAT = .(
	...
	public const VkPrimitiveTopology[(uint32)Topology.MAX_NUM] TOPOLOGIES = .(
	...
	public const VkCullModeFlags[(uint32)CullMode.MAX_NUM] CULL_MODES = .(
	...
	public const VkPolygonMode[(uint32)FillMode.MAX_NUM] POLYGON_MODES = .(
	...
	public static float GetDepthOffset(int32 d)
	{
	...
	public const VkCompareOp[(uint32)CompareFunc.MAX_NUM] COMPARE_OP = .(
	...
	public const VkStencilOp[(uint32)StencilFunc.MAX_NUM] STENCIL_OP = .(
	...
	public const VkBlendFactor[(uint32)BlendFactor.MAX_NUM] BLEND_FACTOR = .(
	...
	public const VkBlendOp[(uint32)BlendFunc.MAX_NUM] BLEND_OP = .(
		.VK_BLEND_OP_ADD,                // ADD
	...
	public const VkImageType[(.)TextureType.MAX_NUM] IMAGE_TYPES = .(
		.VK_IMAGE_TYPE_1D,
		.VK_IMAGE_TYPE_2D,
		.VK_IMAGE_TYPE_3D
		);

	public static VkImageType GetVkImageType(TextureType type)
	{
		return IMAGE_TYPES[(uint32)type];
	}

	public static VkImageUsageFlags GetVkImageUsageFlags(TextureUsageBits usageMask)
	{
		VkImageUsageFlags flags = .VK_IMAGE_USAGE_TRANSFER_SRC_BIT | .VK_IMAGE_USAGE_TRANSFER_DST_BIT;

		if (usageMask & TextureUsageBits.SHADER_RESOURCE != 0)
			flags |= .VK_IMAGE_USAGE_SAMPLED_BIT;

		if (usageMask & TextureUsageBits.SHADER_RESOURCE_STORAGE != 0)
			flags |= .VK_IMAGE_USAGE_STORAGE_BIT;

		if (usageMask & TextureUsageBits.COLOR_ATTACHMENT != 0)
			flags |= .VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;

		if (usageMask & TextureUsageBits.DEPTH_STENCIL_ATTACHMENT != 0)
			flags |= .VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT;

		return flags;
	}

	public static VkImageAspectFlags GetVkImageAspectFlags(Format format)
	{
		switch (format)
		{
		case Format.D16_UNORM: fallthrough;
		case Format.D32_SFLOAT: fallthrough;
		case Format.R24_UNORM_X8: fallthrough;
		case Format.R32_SFLOAT_X8_X24:
			return .VK_IMAGE_ASPECT_DEPTH_BIT;

		case Format.D24_UNORM_S8_UINT: fallthrough;
		case Format.D32_SFLOAT_S8_UINT_X24:
			return .VK_IMAGE_ASPECT_DEPTH_BIT | .VK_IMAGE_ASPECT_STENCIL_BIT;

		case Format.X32_R8_UINT_X24: fallthrough;
	...
	public const VkFilter[(uint32)Filter.MAX_NUM] FILTER = .(
	...
	public const VkSamplerMipmapMode[(uint32)Filter.MAX_NUM] SAMPLER_MIPMAP_MODE = .(
	...
	public const VkSamplerAddressMode[(uint32)AddressMode.MAX_NUM] SAMPLER_ADDRESS_MODE = .(
	...
	public const VkQueryType[(uint32)QueryType.MAX_NUM] QUERY_TYPE = .(
	...
	public static VkQueryType GetVkQueryType(QueryType queryType)
	{
	...
	public const VkImageViewType[(uint32)Texture1DViewType.MAX_NUM] IMAGE_VIEW_TYPE_1D = .(
	...
	public const VkImageViewType[(uint32)Texture2DViewType.MAX_NUM] IMAGE_VIEW_TYPE_2D = .(
	...
	public const VkImageViewType[(uint32)Texture3DViewType.MAX_NUM] IMAGE_VIEW_TYPE_3D = .(
	...
	public const VkImageUsageFlags[(uint32)Texture1DViewType.MAX_NUM] IMAGE_VIEW_USAGE_1D = .(
	...
	public const VkImageUsageFlags[(uint32)Texture2DViewType.MAX_NUM] IMAGE_VIEW_USAGE_2D = .(
	...
	public const VkImageUsageFlags[(uint32)Texture3DViewType.MAX_NUM] IMAGE_VIEW_USAGE_3D = .(
	...
	public const VkImageLayout[(uint32)Texture1DViewType.MAX_NUM] IMAGE_LAYOUT_1D = .(
	...
	public const VkImageLayout[(uint32)Texture2DViewType.MAX_NUM] IMAGE_LAYOUT_2D = .(
	...
	public const VkImageLayout[(uint32)Texture3DViewType.MAX_NUM] IMAGE_LAYOUT_3D = .(
	...
	public static QueryType GetQueryType(VkQueryType type)
	{
	...
	{
		if ((int)type < TEXTURE_TYPE_TABLE.Count)
	...
	public static Result GetReturnCode(VkResult result)
	{
		...
		case .VK_ERROR_SURFACE_LOST_KHR: fallthrough;
		case .VK_ERROR_OUT_OF_DATE_KHR:
			return Result.SWAPCHAIN_RESIZE;

		case .VK_ERROR_NATIVE_WINDOW_IN_USE_KHR: fallthrough;
		case .VK_ERROR_INCOMPATIBLE_DISPLAY_KHR: fallthrough;
		case .VK_ERROR_FORMAT_NOT_SUPPORTED: fallthrough;
		case .VK_ERROR_INCOMPATIBLE_DRIVER: fallthrough;
		case .VK_ERROR_FEATURE_NOT_PRESENT: fallthrough;
		case .VK_ERROR_EXTENSION_NOT_PRESENT: fallthrough;
		case .VK_ERROR_LAYER_NOT_PRESENT:
			return Result.UNSUPPORTED;

		case .VK_ERROR_INVALID_EXTERNAL_HANDLE: fallthrough;
		case .VK_ERROR_VALIDATION_FAILED_EXT: fallthrough;
		case .VK_ERROR_INVALID_SHADER_NV: fallthrough;
		case .VK_ERROR_NOT_PERMITTED_KHR: fallthrough;
		case .VK_SUBOPTIMAL_KHR: fallthrough;
		case .VK_ERROR_TOO_MANY_OBJECTS: fallthrough;
		case .VK_ERROR_MEMORY_MAP_FAILED: fallthrough;
		case .VK_ERROR_INITIALIZATION_FAILED: fallthrough;
		case .VK_INCOMPLETE: fallthrough;
		case .VK_TIMEOUT: fallthrough;
		case .VK_NOT_READY:
			return Result.FAILURE;

		case .VK_ERROR_OUT_OF_HOST_MEMORY: fallthrough;
		case .VK_ERROR_OUT_OF_DEVICE_MEMORY: fallthrough;
		case .VK_ERROR_OUT_OF_POOL_MEMORY: fallthrough;
		case .VK_ERROR_FRAGMENTATION: fallthrough;
