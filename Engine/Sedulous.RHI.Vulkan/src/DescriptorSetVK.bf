...
namespace Sedulous.RHI.Vulkan
{
	struct SlabAllocator
	{
		...
		public this(void* memory, int size)
		{
			m_CurrentOffset = (uint8*)memory;
			m_End = (int)memory + size;
			m_Memory = (uint8*)memory;
		}

		public T* Allocate<T>(ref uint32 number) mut where T : struct
		{
			T* items = (T*)Align(m_CurrentOffset, alignof(T));
			readonly int itemsLeft = (m_End - (int)(void*)items) / sizeof(T);
	...
	class DescriptorSetVK : DescriptorSet
	{
		...
		private DeviceVK m_Device;
		 ...
		 /////////////////////////////Internal Methods//////////////////////////////
		public VkDescriptorSet GetHandle(uint32 physicalDeviceIndex) => m_Handles[physicalDeviceIndex];
		...
		public override void Copy(DescriptorSetCopyDesc descriptorSetCopyDesc)
		{
			...
			vkUpdateDescriptorSets(m_Device, 0, null, copyNum, copies);
		}
	}
}