...
namespace Sedulous.RHI.Vulkan
{
	class MemoryVK : Memory
	{
		...
		public Result Create(uint32 physicalDeviceMask, MemoryType memoryType, uint64 size)
		{
			...
			for (uint32 i = 0; i < m_Device.GetPhyiscalDeviceGroupSize(); i++)
			{
				if ((1 << i) & physicalDeviceMask != 0)
				{
					...
					VkResult result = vkAllocateMemory(m_Device, &memoryInfo, m_Device.GetAllocationCallbacks(), &m_Handles[i]);
					...
					if (IsHostVisibleMemory(memoryLocation))
					{
						result = vkMapMemory(m_Device, m_Handles[i], 0, size, 0, (void**)&m_MappedMemory[i]);
		...
		public Result Create(MemoryVulkanDesc memoryDesc)
		{
			...
			for (uint32 i = 0; i < m_Device.GetPhyiscalDeviceGroupSize(); i++)
			{
				if ((1 << i) & physicalDeviceMask != 0)
				{
					...
					{
						readonly VkResult result = vkMapMemory(m_Device, m_Handles[i], 0, memoryDesc.size, 0, (void**)&m_MappedMemory[i]);
		...
		public Result CreateDedicated(BufferVK buffer, uint32 physicalDeviceMask)
		{
			...
			RETURN_ON_FAILURE!(m_Device.GetLogger(), memoryTypeInfo.isDedicated, Result.FAILURE,
			...
			for (uint32 i = 0; i < m_Device.GetPhyiscalDeviceGroupSize(); i++)
			{
				if ((1 << i) & physicalDeviceMask != 0)
				{
					...
					if (IsHostVisibleMemory(memoryLocation))
					{
						result = vkMapMemory(m_Device, m_Handles[i], 0, memoryDesc.size, 0, (void**)&m_MappedMemory[i]);
		...
		public VkDeviceMemory GetHandle(uint32 physicalDeviceIndex) => m_Handles[physicalDeviceIndex];
		...
		public ~this()
		{
			...
			{
				if (m_Handles[i] != .Null)
					vkFreeMemory(m_Device, m_Handles[i], m_Device.GetAllocationCallbacks());
			}
		}
