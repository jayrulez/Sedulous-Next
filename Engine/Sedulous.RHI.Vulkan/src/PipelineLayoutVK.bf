...
namespace Sedulous.RHI.Vulkan
{
	...
	class PipelineLayoutVK : PipelineLayout
	{
		...
		private void CreateSetLayout(in DescriptorSetDesc descriptorSetDesc, in uint32* bindingOffsets)
		{
			...
			readonly VkResult result = vkCreateDescriptorSetLayout(m_Device, &info, m_Device.GetAllocationCallbacks(), &handle);
		...
		private void FillRuntimeBindingInfo(in PipelineLayoutDesc pipelineLayoutDesc, in uint32* bindingOffsets)
		{
			...
			destination.descriptorSetDescs.Insert(0, Span<DescriptorSetDesc>(source.descriptorSets, (int)source.descriptorSetNum));

			destination.pushConstantDescs.Insert(0, Span<PushConstantDesc>(source.pushConstants, (int)source.pushConstantNum));

			destination.pushConstantBindings.Resize(source.pushConstantNum);
			for (uint32 i = 0, offset = 0; i < source.pushConstantNum; i++)
			{
			    destination.pushConstantBindings[i] = .()
					{
						flags = VulkanUtils.GetShaderStageFlags(source.pushConstants[i].visibility),
						offset = offset
					};
			...
			for (uint32 i = 0; i < source.descriptorSetNum; i++)
			{
			    ...
			    destination.descriptorSetRangeDescs.AddRange(Span<DescriptorRangeDesc>(descriptorSetDesc.ranges, (int)descriptorSetDesc.rangeNum));
			    ...
			    destination.dynamicConstantBufferDescs.AddRange(Span<DynamicConstantBufferDesc>(descriptorSetDesc.dynamicConstantBuffers, (int)descriptorSetDesc.dynamicConstantBufferNum));
		...
		public Result Create(in PipelineLayoutDesc pipelineLayoutDesc)
		{
			...
			readonly VkResult result = vkCreatePipelineLayout(m_Device, &pipelineLayoutCreateInfo, m_Device.GetAllocationCallbacks(), &m_Handle);
		...
		public this(DeviceVK device)
		{
			...
			m_RuntimeBindingInfo = .(m_Device.GetDeviceAllocator());
			m_DescriptorSetLayouts = Allocate!<List<VkDescriptorSetLayout>>(m_Device.GetDeviceAllocator());
			m_StaticSamplers = Allocate!<List<DescriptorVK>>(m_Device.GetDeviceAllocator());
		}

		public ~this()
		{
			readonly ref VkAllocationCallbacks* allocationCallbacks = ref m_Device.GetAllocationCallbacks();

			if (m_Handle != .Null)
				vkDestroyPipelineLayout(m_Device, m_Handle, allocationCallbacks);

			for (ref VkDescriptorSetLayout handle in ref m_DescriptorSetLayouts)
				vkDestroyDescriptorSetLayout(m_Device, handle, allocationCallbacks);

			m_StaticSamplers.Clear();

			Deallocate!(m_Device.GetDeviceAllocator(), m_StaticSamplers);
			Deallocate!(m_Device.GetDeviceAllocator(), m_DescriptorSetLayouts);
