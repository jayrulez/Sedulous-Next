...
namespace Sedulous.RHI.Vulkan
{
	...
	class DescriptorPoolVK : DescriptorPool
	{
		...
		public Result Create(DescriptorPoolDesc descriptorPoolDesc)
		{
			...
			readonly VkResult result = vkCreateDescriptorPool(m_Device, &info, m_Device.GetAllocationCallbacks(), &m_Handle);
		...
		public Result Create(VkDescriptorPool vkDescriptorPool)
		{
		...
		public this(DeviceVK device)
		{
			...
			m_AllocatedSets = Allocate!<List<DescriptorSetVK>>(m_Device.GetDeviceAllocator());
		...
		public ~this()
		{
			...
			{
				Deallocate!(m_Device.GetDeviceAllocator(), m_AllocatedSets[i]);
			}

			if (m_Handle != .Null && m_OwnsNativeObjects)
				vkDestroyDescriptorPool(m_Device, m_Handle, m_Device.GetAllocationCallbacks());
		...
		public override void Reset()
		{
			...
			readonly VkResult result = vkResetDescriptorPool(m_Device, m_Handle, ( /*VkDescriptorPoolResetFlags*/.)0);
