...
namespace Sedulous.Graphics
{
	...
	public abstract class CommandBuffer : IDisposable
	{
		...
		public abstract void SetVertexBuffer(uint32 slot, Buffer buffer, uint32 offset);
		...
		public abstract void SetVertexBuffers(Buffer[] buffers, int32[] offsets);
		...
		public abstract void SetIndexBuffer(Buffer buffer, IndexFormat format = IndexFormat.UInt16, uint32 offset = 0u);

		/// <summary>
		/// Set an scissor rectangle in a specific slot.
		/// </summary>
		/// <param name="rectangles">The array of the scissor rectangles.</param>
		public abstract void SetScissorRectangles(Rect[] rectangles);
		...
		/// <summary>
		/// Set the active <see cref="T:Sedulous.Graphics.ResourceSet" /> for the given index.
		/// </summary>
		/// <param name="resourceSet">The new <see cref="T:Sedulous.Graphics.ResourceSet" />.</param>
		...
		public abstract void SetResourceSet(ResourceSet resourceSet, uint32 index = 0u, uint32[] constantBufferOffsets = null);

		/// <summary>
		/// Begin a render pass.
		/// </summary>
		/// <param name="description">The renderPass description <see cref="T:Sedulous.Graphics.RenderPassDescription" />.</param>
		...
		public void UpdateBufferData<T>(Buffer buffer, T[] data, uint32 destinationOffsetInBytes = 0u) where T : struct
		{
			UpdateBufferData(buffer, data, (data != null) ? ((uint32)data.Count) : 0u, destinationOffsetInBytes);
		}
		...
		/// <param name="destinationOffsetInBytes">The destination offset.</param>
		public void UpdateBufferData<T>(Buffer buffer, T[] data, uint32 count, uint32 destinationOffsetInBytes = 0u) where T : struct
		{
			uint32 sourceSizeInBytes = count * (.)sizeof(T);
			UpdateBufferData(buffer, data.Ptr, sourceSizeInBytes, destinationOffsetInBytes);
		}
		...
		/// <param name="data">The data array.</param>
		/// <param name="destinationOffsetInBytes">The destination offset.</param>
		public  void UpdateBufferData<T>(Buffer buffer, ref T data, uint32 destinationOffsetInBytes = 0u) where T : struct
		{
			uint32 sourceSizeInBytes = (.)sizeof(T);
			UpdateBufferData(buffer, &data, sourceSizeInBytes, destinationOffsetInBytes);
		}
		...
		public void UpdateBufferData(Buffer buffer, void* source, uint32 sourceSizeInBytes, uint32 destinationOffsetInBytes = 0u)
		{
			if (buffer != null)
			{
		...
		/// </summary>
		/// <param name="source">The source <see cref="T:Sedulous.Graphics.Texture"></see>.</param>
		/// <param name="destination">The destination <see cref="T:Sedulous.Graphics.Texture"></see> into which data is copied./&gt;.</param>
		public void CopyTextureDataTo(Texture source, Texture destination)
		{
			for (uint32 num = 0u; num < source.Description.MipLevels; num++)
			{
				Helpers.GetMipDimensions(source.Description, num, var width, var height, var depth);
				CopyTextureDataTo(source, 0, 0, 0, num, 0, destination, 0, 0, 0, num, 0, width, height, depth, source.Description.ArraySize * source.Description.Faces);
			}
		}
		...
		{
			Helpers.GetMipDimensions(source.Description, mipLevel, var width, var height, var depth);
			CopyTextureDataTo(source, 0u, 0u, 0u, mipLevel, arrayLayer, destination, 0u, 0u, 0u, mipLevel, arrayLayer, width, height, depth, 1u);
		}
		...
		{
			Dispatch(MathHelper.DivideByMultiple(threadCountX, groupSizeX), 1, 1);
		}
		...
		{
			Dispatch(MathHelper.DivideByMultiple(threadCountX, groupSizeX), MathHelper.DivideByMultiple(threadCountY, groupSizeY), 1);
		}
		...
		{
			Dispatch(MathHelper.DivideByMultiple(threadCountX, groupSizeX), MathHelper.DivideByMultiple(threadCountY, groupSizeY), MathHelper.DivideByMultiple(threadCountZ, groupSizeZ));
		}

		/// <summary>
		/// Commits this command buffer to the command queue waiting to be execute on GPU after <see cref="M:Sedulous.Graphics.CommandQueue.Submit" />.
		/// </summary>
		...
		/// </summary>
		/// <param name="argBuffer">A buffer which must be loaded with data that matches the argument list for <see cref="M:Sedulous.Graphics.CommandBuffer.Dispatch(System.UInt32,System.UInt32,System.UInt32)" />.</param>
		/// <param name="offset">A uint8-aligned offset between the start of the buffer and the arguments.</param>
		public abstract void DispatchIndirect(Buffer argBuffer, uint32 offset);

		/// <summary>
		/// Generates mipmaps for the given <see cref="T:Sedulous.Graphics.Texture" />. The largest mipmap is used to generate all of the lower mipmap
		/// levels contained in the Texture.
		/// </summary>
		/// <param name="texture">The <see cref="T:Sedulous.Graphics.Texture" /> to generate mipmaps for. This Texture must have been created with
		/// <see cref="T:Sedulous.Graphics.TextureFlags" />.<see cref="F:Sedulous.Graphics.TextureFlags.GenerateMipmaps" />.</param>
