...
namespace Sedulous.Graphics
{
	...
	public abstract class GraphicsContext : IDisposable
	{
		...
		public  void UpdateBufferData<T>(Buffer buffer, ref T data, uint32 destinationOffsetInBytes = 0) where T : struct
		{
			...
			void* source = &data;
			UpdateBufferData(buffer, source, sourceSizeInBytes, destinationOffsetInBytes);
		}
		...
		{
			uint32 sourceSizeInBytes = count * (.)sizeof(T);
			UpdateTextureData(texture, data.Ptr, sourceSizeInBytes, destinationOffsetInBytes);
		}
		...
		public  void UpdateTextureData<T>(Texture texture, ref T data, uint32 destinationOffsetInBytes = 0) where T : struct
		{
			uint32 sourceSizeInBytes = (.)sizeof(T);
			void* source = &data;
			UpdateTextureData(texture, source, sourceSizeInBytes, destinationOffsetInBytes);
		}
		...
		/// <summary>
		/// Maps a <see cref="T:Sedulous.Graphics.Buffer" /> or <see cref="T:Sedulous.Graphics.Texture" /> into a CPU-accessible data region.
		/// </summary>
		/// <param name="resource">The graphics resource to map.</param>
		/// <param name="mode">The <see cref="T:Sedulous.Graphics.MapMode" /> used to map the resource.</param>
		...
		/// <returns>A <see cref="T:Sedulous.Graphics.MappedResource" /> structure describing the mapped data region.</returns>
		public abstract MappedResource MapMemory(GraphicsResource resource, MapMode mode, uint32 subResource = 0);

		/// <summary>
		/// Invalidates a previously-mapped data region for the given <see cref="T:Sedulous.Graphics.Buffer" /> or <see cref="T:Sedulous.Graphics.Texture" />.
		/// </summary>
		/// <param name="resource">The graphics resource to map.</param>
		/// <param name="subResource">The subresource to map. Subresources are indexed first by mip slice and then by array layer. Only for Textures.</param>
		...
		/// <param name="stage">The shader stage, <see cref="T:Sedulous.Graphics.ShaderStages" />.</param>
		...
		protected virtual void CreateDefaultSampler()
		{
			if (DefaultSampler == null)
			{
				...
				DefaultSampler = Factory.CreateSamplerState(ref description);
			}
		}
	}
}
