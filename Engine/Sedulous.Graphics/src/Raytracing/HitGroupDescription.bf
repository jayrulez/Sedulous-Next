...
namespace Sedulous.Graphics.Raytracing
{
	...
	/// </summary>
	public struct HitGroupDescription : IHashable
	{
		/// <summary>
		/// Hit Group Type.
		/// </summary>
		...
		public int GetHashCode()
		{
			...
			hash = HashHelper.CombineHash(hash, Name?.GetHashCode() ?? 0);
			hash = HashHelper.CombineHash(hash, GeneralEntryPoint?.GetHashCode() ?? 0);
			hash = HashHelper.CombineHash(hash, ClosestHitEntryPoint?.GetHashCode() ?? 0);
			hash = HashHelper.CombineHash(hash, AnyHitEntryPoint?.GetHashCode() ?? 0);
			hash = HashHelper.CombineHash(hash, IntersectionEntryPoint?.GetHashCode() ?? 0);
