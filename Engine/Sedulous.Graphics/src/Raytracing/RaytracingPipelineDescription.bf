...
namespace Sedulous.Graphics.Raytracing
{
	...
	public struct RaytracingPipelineDescription : IEquatable<RaytracingPipelineDescription>
	{
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="T:Sedulous.Graphics.Raytracing.RaytracingPipelineDescription" /> struct.
		/// </summary>
		...
		{
			int hash = Shaders.GetHashCode();

			hash = HashHelper.CombineHash(hash, ResourceLayouts.GetHashCode());
			hash = HashHelper.CombineHash(hash, HitGroups.GetHashCode());
			hash = HashHelper.CombineHash(hash, (.)MaxTraceRecursionDepth);
			hash = HashHelper.CombineHash(hash, (.)MaxPayloadSizeInBytes);
			hash = HashHelper.CombineHash(hash, (.)MaxAttributeSizeInBytes);
