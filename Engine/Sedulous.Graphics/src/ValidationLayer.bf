...
namespace Sedulous.Graphics
{
	...
	public class ValidationLayer
	{
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="T:Sedulous.Graphics.ValidationLayer" /> class.
		/// </summary>
		/// <param name="method">The notify method <see cref="T:Sedulous.Graphics.ValidationLayer.NotifyMethod" />, exception by default.</param>
		...
		{
			if (description.Outputs.ColorAttachments == null && !description.Outputs.DepthAttachment.HasValue)
			{
		...
		public void CreateBufferValidation(void* data, ref BufferDescription description)
		{
			...
			if ((description.Flags & BufferFlags.ConstantBuffer) != 0 && description.SizeInBytes % 16u != 0)
			{
		...
		{
			if (this.Error.HasListeners)
			{
				String e = scope $"A {owner} error was encountered: {message}";
				this.Error.Invoke(severity, e);
			}
		}
		...
		{
			String errorMessage = scope $"A {owner} error was encountered: {message}";
			switch (severity)
			{
