...
namespace Sedulous.Graphics
{
	...
	public struct FrameBufferAttachment : IEquatable<FrameBufferAttachment>
	{
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="T:Sedulous.Graphics.FrameBufferAttachment" /> struct.
		/// </summary>
		...
		public this(Texture attachedTexture, uint32 arrayIndex, uint32 faceIndex, uint32 sliceCount, uint32 mipLevel)
			: this(attachedTexture, arrayIndex * attachedTexture.Description.Faces + faceIndex, null, 0u, sliceCount, mipLevel)
		{
		}
		...
		/// <param name="sliceCount">The slice count.</param>
		/// <param name="mipLevel">The selected mipLevel.</param>
		public this(Texture attachedTexture, uint32 firstSlice, uint32 sliceCount, uint32 mipLevel = 0u)
			: this(attachedTexture, firstSlice, null, 0u, sliceCount, mipLevel)
		{
		}
		...
		public this(Texture attachedTexture, Texture resolvedTexture)
			: this(attachedTexture, 0u, resolvedTexture, 0u, 1u, 0u)
		{
		}
		...
		public this(Texture attachedTexture, uint32 attachedFirstSlice, Texture resolvedTexture, uint32 resolvedFirstSlice, uint32 sliceCount, uint32 mipLevel)
		{
			if (resolvedTexture != null)
			{
				...
				{
					Runtime.FatalError("The resolved texture must have SampleCount == None");
				}
				if (attachedTexture.Description.SampleCount == TextureSampleCount.None)
				{
					Runtime.FatalError("Framebuffer attachment texture must have SampleCount != None if there are a resolvedTexture");
				}
			}
		...
		{
			int hash = (.)(void*)&AttachmentTexture;
			hash = HashHelper.CombineHash(hash, (.)AttachedFirstSlice);
			hash = HashHelper.CombineHash(hash, (.)SliceCount);
			hash = HashHelper.CombineHash(hash, (.)MipSlice);
			if (ResolvedTexture != null)
			{
				hash = HashHelper.CombineHash(hash, (.)(void*)&ResolvedTexture);
				hash = HashHelper.CombineHash(hash, (.)ResolvedFirstSlice);
			}
			return hash;
		}
