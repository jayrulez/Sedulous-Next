...
namespace Sedulous.GAL
{
    ...
    public struct GraphicsPipelineDescription : IEquatable<GraphicsPipelineDescription>, IHashable
    {
        ...
        public Span<ResourceLayout> ResourceLayouts;
        /// <summary>
            ...
            OutputDescription outputs)
        {
            ...
            ResourceLayouts = Span<ResourceLayout>(&resourceLayout, 1);
            Outputs = outputs;
            ResourceBindingModel = null;
        }
        ...
        public bool Equals(GraphicsPipelineDescription other)
        {
			...
			for(int i = 0; i < ResourceLayouts.Length; i++){
				if(ResourceLayouts[i] != other.ResourceLayouts[i])
					return false;
			}
        ...
        public int GetHashCode()
        {
			...
			int hash = HashHelper.Combine(BlendState.GetHashCode(), DepthStencilState.GetHashCode(),
                RasterizerState.GetHashCode(),
                (int)PrimitiveTopology,
                ShaderSet.GetHashCode());

			for(var resourceLayout in ResourceLayouts){
				hash = HashHelper.Combine(hash, resourceLayout.GetHashCode());
			}
