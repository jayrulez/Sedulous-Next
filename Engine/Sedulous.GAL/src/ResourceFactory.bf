...
namespace Sedulous.GAL
{
    ...
    public abstract class ResourceFactory
    {
        ...
        /// <returns></returns>
        protected abstract Pipeline CreateGraphicsPipelineCore(GraphicsPipelineDescription description);
        ...
        public abstract Pipeline CreateComputePipeline(ComputePipelineDescription description);
        ...
        public abstract Framebuffer CreateFramebuffer(FramebufferDescription description);
        ...
        public Texture CreateTexture(TextureDescription description)
        {
            ...
            {
                Runtime.FatalError("Width, Height, and Depth must be non-zero.");
            }
            ...
            {
                Runtime.FatalError("The givel PixelFormat can only be used in a Texture with DepthStencil usage.");
            }
            ...
            {
				//Runtime.FatalError(scope $"1D and 3D Textures must use {typeof(TextureSampleCount).GetName(.. scope .())}.{nameof(TextureSampleCount.Count1)}.");
                Runtime.FatalError(scope $"1D and 3D Textures must use {typeof(TextureSampleCount).GetName(.. scope .())}..Count1.");
            }
            if (description.Type == TextureType.Texture1D && !Features.Texture1D)
            {
                Runtime.FatalError($"1D Textures are not supported by this device.");
            }
            if ((description.Usage & TextureUsage.Staging) != 0 && description.Usage != TextureUsage.Staging)
            {
				//Runtime.FatalError(scope $"{typeof(TextureUsage).GetName(.. scope .())}.{nameof(TextureUsage.Staging)} cannot be combined with any other flags.");
                Runtime.FatalError(scope $"{typeof(TextureUsage).GetName(.. scope .())}.Staging cannot be combined with any other flags.");
            }
            if ((description.Usage & TextureUsage.DepthStencil) != 0 && (description.Usage & TextureUsage.GenerateMipmaps) != 0)
            {
				//Runtime.FatalError(scope $"{typeof(TextureUsage).GetName(.. scope .())}.{nameof(TextureUsage.DepthStencil)} and {typeof(TextureUsage).GetName(.. scope .())}.{nameof(TextureUsage.GenerateMipmaps)} cannot be combined.");
                Runtime.FatalError(scope $"{typeof(TextureUsage).GetName(.. scope .())}..DepthStencil and {typeof(TextureUsage).GetName(.. scope .())}..GenerateMipmaps cannot be combined.");
            }
#endif
        ...
        /// <param name="description"></param>
        /// <returns></returns>
        protected abstract Texture CreateTextureCore(uint64 nativeTexture, TextureDescription description);
        ...
        /// <param name="description"></param>
        /// <returns></returns>
        protected abstract Texture CreateTextureCore(TextureDescription description);
        ...
        public TextureView CreateTextureView(TextureViewDescription description)
        {
            ...
            {
                Runtime.FatalError("GraphicsDevice does not support subset TextureViews.");
            }
            if (description.Format != null && description.Format != description.Target.Format)
            {
                ...
                {
                    Runtime.FatalError(scope $"Cannot create a TextureView with format {description.Format.Value} targeting a Texture with format {description.Target.Format}. A TextureView's format must have the same size and number of components as the underlying Texture's format, or the same format.");
                }
            }
#endif
        ...
        /// <param name="description"></param>
        /// <returns></returns>
        protected abstract TextureView CreateTextureViewCore(TextureViewDescription description);
        ...
        public DeviceBuffer CreateBuffer(BufferDescription description)
        {
                ...
                || (usage & BufferUsage.StructuredBufferReadWrite) == BufferUsage.StructuredBufferReadWrite)
            {
                ...
                {
                    Runtime.FatalError("GraphicsDevice does not support structured buffers.");
                }

                if (description.StructureByteStride == 0)
                {
                    Runtime.FatalError("Structured Buffer objects must have a non-zero StructureByteStride.");
                }

                if ((usage & BufferUsage.StructuredBufferReadWrite) != 0 && usage != BufferUsage.StructuredBufferReadWrite)
                {
					//Runtime.FatalError(scope $"{typeof(BufferUsage).GetName(.. scope .())}.{nameof(BufferUsage.StructuredBufferReadWrite)} cannot be combined with any other flag.");
                    Runtime.FatalError(scope $"{typeof(BufferUsage).GetName(.. scope .())}..StructuredBufferReadWrite cannot be combined with any other flag.");
                }
                else if ((usage & BufferUsage.VertexBuffer) != 0
                    || (usage & BufferUsage.IndexBuffer) != 0
                    || (usage & BufferUsage.IndirectBuffer) != 0)
                {
					//Runtime.FatalError(scope $"Read-Only Structured Buffer objects cannot specify {typeof(BufferUsage).GetName(.. scope .())}.{nameof(BufferUsage.VertexBuffer)}, {typeof(BufferUsage).GetName(.. scope .())}.{nameof(BufferUsage.IndexBuffer)}, or {typeof(BufferUsage).GetName(.. scope .())}.{nameof(BufferUsage.IndirectBuffer)}.");
                    Runtime.FatalError(scope $"Read-Only Structured Buffer objects cannot specify {typeof(BufferUsage).GetName(.. scope .())}.VertexBuffer, {typeof(BufferUsage).GetName(.. scope .())}.IndexBuffer, or {typeof(BufferUsage).GetName(.. scope .())}.IndirectBuffer.");
                }
            }
            else if (description.StructureByteStride != 0)
            {
                Runtime.FatalError("Non-structured Buffers must have a StructureByteStride of zero.");
            }
            if ((usage & BufferUsage.Staging) != 0 && usage != BufferUsage.Staging)
            {
                Runtime.FatalError("Buffers with Staging Usage must not specify any other Usage flags.");
            }
            if ((usage & BufferUsage.UniformBuffer) != 0 && (description.SizeInBytes % 16) != 0)
            {
                Runtime.FatalError($"Uniform buffer size must be a multiple of 16 bytes.");
            }
#endif
        ...
        /// <param name="description"></param>
        /// <returns></returns>
        protected abstract DeviceBuffer CreateBufferCore(BufferDescription description);
        ...
        /// <param name="description"></param>
        /// <returns></returns>
        protected abstract Sampler CreateSamplerCore(SamplerDescription description);
        ...
        public Shader CreateShader(ShaderDescription description)
        {
            ...
            {
                Runtime.FatalError("GraphicsDevice does not support Compute Shaders.");
            }
            ...
            {
                Runtime.FatalError("GraphicsDevice does not support Tessellation Shaders.");
            }
#endif
        ...
        /// <param name="description"></param>
        /// <returns></returns>
        protected abstract Shader CreateShaderCore(ShaderDescription description);
        ...
        /// <param name="description">The desired properties of the created object.</param>
        /// <returns>A new <see cref="CommandList"/>.</returns>
        public abstract CommandList CreateCommandList(CommandListDescription description);
