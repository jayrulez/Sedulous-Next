...
namespace Sedulous.GAL
{
    ...
    public struct FramebufferAttachmentDescription : IEquatable<FramebufferAttachmentDescription>, IHashable
    {
        ...
        public this(Texture target, uint32 arrayLayer, uint32 mipLevel)
        {
            ...
            {
				//Runtime.FatalError(scope $"{nameof(arrayLayer)} must be less than {nameof(target)}.{nameof(Texture.ArrayLayers)}.");
                Runtime.FatalError(scope $"arrayLayer must be less than target..ArrayLayers.");
            }
            if (mipLevel >= target.MipLevels)
            {
				//Runtime.FatalError(scope $"{nameof(mipLevel)} must be less than {nameof(target)}.{nameof(Texture.MipLevels)}.");
                Runtime.FatalError(scope $"mipLevel must be less than target.MipLevels.");
            }
#endif
            Target = target;
        ...
        {
			int hash = (int)Internal.UnsafeCastToPtr(Target);
            return HashHelper.Combine(hash, ArrayLayer.GetHashCode(), MipLevel.GetHashCode());
        }
    }
}
