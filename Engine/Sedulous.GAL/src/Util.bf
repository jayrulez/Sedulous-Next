...
namespace Sedulous.GAL
{
    ...
    internal static class Util
    {
			...
			where TBase : class
        {
            ...
            if (value == null)
            {
                Runtime.FatalError(scope $"Expected object of type {typeof(TDerived).GetFullName(.. scope .())} but received null instead.");
            }

            if (TDerived derived = value as TDerived )
            {
            	return derived;
            }

			Runtime.FatalError(scope $"object {value} must be derived type {typeof(TDerived).GetFullName(.. scope .())} to be used in this context.");
        ...
        internal static void ShallowClone<T>(T[] src, out T[] dst)
        {
			dst = new T[src.Count];
        ...
        {
            if (DeviceBufferRange range = resource as DeviceBufferRange?)
            {
        ...
        {
            if (DeviceBuffer db = resource as DeviceBuffer)
            {
                buffer = db;
                return true;
            }
            else if (DeviceBufferRange range = resource as DeviceBufferRange?)
            {
                buffer = range.Buffer;
                return true;
            }
        ...
        {
            if (TextureView view = resource as TextureView)
            {
                return view;
            }
            else if (Texture tex = resource as Texture)
            {
                return tex.GetFullTextureView(gd);
            }
            else
            {
                Runtime.FatalError(scope $"Unexpected resource type. Expected Texture or TextureView but found {resource.GetType().GetFullName(.. scope .())}");
            }
        }
