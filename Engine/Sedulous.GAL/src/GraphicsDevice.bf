    ...
    public abstract class GraphicsDevice : IDisposable
    {
        ...
        public virtual bool SyncToVerticalBlank
        {
            ...
            set
            {
                ...
                {
                    Runtime.FatalError($"This GraphicsDevice was created without a main Swapchain. This property cannot be set.");
                }
        ...
        internal protected abstract uint32 GetUniformBufferMinOffsetAlignmentCore();
        internal protected abstract uint32 GetStructuredBufferMinOffsetAlignmentCore();
        ...
        internal protected abstract void SubmitCommandsCore(
        ...
        public void WaitForFence(Fence fence)
        {
            ...
            {
                Runtime.FatalError("The operation timed out before the Fence was signaled.");
            }
        }
        ...
        public void WaitForFences(Fence[] fences, bool waitAll)
        {
            ...
            {
                Runtime.FatalError("The operation timed out before the Fence(s) were signaled.");
            }
        }
        ...
        public void SwapBuffers()
        {
            if (MainSwapchain == null)
            {
                Runtime.FatalError("This GraphicsDevice was created without a main Swapchain, so the requested operation cannot be performed.");
            }
        ...
        internal protected abstract void SwapBuffersCore(Swapchain swapchain);
        ...
        internal protected abstract void WaitForIdleCore();
        ...
        {
#if VALIDATE_USAGE
            if (var buffer = resource as DeviceBuffer)
            {
                if ((buffer.Usage & BufferUsage.Dynamic) != BufferUsage.Dynamic
                    && (buffer.Usage & BufferUsage.Staging) != BufferUsage.Staging)
                {
                    Runtime.FatalError("Buffers must have the Staging or Dynamic usage flag to be mapped.");
                }
                if (subresource != 0)
                {
                    Runtime.FatalError("Subresource must be 0 for Buffer resources.");
                }
                if ((mode == MapMode.Read || mode == MapMode.ReadWrite) && (buffer.Usage & BufferUsage.Staging) == 0)
                {
					//Runtime.FatalError(scope $"{nameof(MapMode)}.{nameof(MapMode.Read)} and {nameof(MapMode)}.{nameof(MapMode.ReadWrite)} can only be used on buffers created with {nameof(BufferUsage)}.{nameof(BufferUsage.Staging)}.");
                    Runtime.FatalError(scope $"MapMode..Read and MapMode.ReadWrite can only be used on buffers created with BufferUsage.Staging.");
                }
            }
            else if (var  tex = resource as Texture)
            {
                if ((tex.Usage & TextureUsage.Staging) == 0)
                {
                    Runtime.FatalError("Texture must have the Staging usage flag to be mapped.");
                }
        ...
        internal protected abstract void UpdateTextureCore(
            Texture texture,
            void* source,
            uint32 sizeInBytes,
            uint32 x, uint32 y, uint32 z,
            ...
            uint32 mipLevel, uint32 arrayLayer)
        {
            if (FormatHelpers.IsCompressedFormat(texture.Format))
            {
                if (x % 4 != 0 || y % 4 != 0 || height % 4 != 0 || width % 4 != 0)
                {
                    ...
                    {
                        Runtime.FatalError($"Updates to block-compressed textures must use a region that is block-size aligned and sized.");
                    }
                }
            }
            ...
            {
                Runtime.FatalError($"The given region does not fit into the Texture.");
            }

            if (mipLevel >= texture.MipLevels)
            {
				//Runtime.FatalError(scope $"{nameof(mipLevel)} ({mipLevel}) must be less than the Texture's mip level count ({texture.MipLevels}).");
                Runtime.FatalError(scope $"mipLevel ({mipLevel}) must be less than the Texture's mip level count ({texture.MipLevels}).");
            }
            ...
            {
				//Runtime.FatalError(scope $"{nameof(arrayLayer)} ({arrayLayer}) must be less than the Texture's effective array layer count ({effectiveArrayLayers}).");
                Runtime.FatalError(scope $"arrayLayer ({arrayLayer}) must be less than the Texture's effective array layer count ({effectiveArrayLayers}).");
            }
        }
        ...
        internal protected abstract void UpdateBufferCore(DeviceBuffer buffer, uint32 bufferOffsetInBytes, void* source, uint32 sizeInBytes);
        ...
        internal protected abstract bool GetPixelFormatSupportCore(
            PixelFormat format,
            TextureType type,
            TextureUsage usage,
            out PixelFormatProperties properties);
        ...
        public void Dispose()
        {
			...
			if(_aniso4xSampler != null)
				delete _aniso4xSampler;
            PlatformDispose();
        }
	}
}
