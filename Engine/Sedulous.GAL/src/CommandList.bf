...
namespace Sedulous.GAL
{
    ...
    public abstract class CommandList : DeviceResource, IDisposable
    {
        ...
        internal protected Framebuffer _framebuffer;
        internal protected Pipeline _graphicsPipeline;
        internal protected Pipeline _computePipeline;
        ...
        internal protected abstract void SetPipelineCore(Pipeline pipeline);
        ...
        internal protected abstract void SetVertexBufferCore(uint32 index, DeviceBuffer buffer, uint32 offset);
        ...
        internal protected abstract void SetIndexBufferCore(DeviceBuffer buffer, IndexFormat format, uint32 offset);
        ...
        public void SetGraphicsResourceSet(uint32 slot, ResourceSet rs, uint32 dynamicOffsetsCount, uint32* dynamicOffsets)
        {
            ...
            {
				//Runtime.FatalError(scope $"A graphics Pipeline must be active before {nameof(SetGraphicsResourceSet)} can be called.");
                Runtime.FatalError(scope $"A graphics Pipeline must be active before SetGraphicsResourceSet can be called.");
            }
            ...
            {
                Runtime.FatalError(scope $"Failed to bind ResourceSet to slot {slot}. The number of resources in the ResourceSet ({setLength}) does not match the number expected by the active Pipeline ({pipelineLength}).");
            }
            ...
            {
				//Runtime.FatalError(scope $"A dynamic offset must be provided for each resource that specifies {nameof(ResourceLayoutElementOptions)}.{nameof(ResourceLayoutElementOptions.DynamicBinding)}. {rs.Layout.DynamicBufferCount} offsets were expected, but only {dynamicOffsetsCount} were provided.");
                Runtime.FatalError(scope $"A dynamic offset must be provided for each resource that specifies ResourceLayoutElementOptions.DynamicBinding. {rs.Layout.DynamicBufferCount} offsets were expected, but only {dynamicOffsetsCount} were provided.");
            }
            ...
            for (uint32 i = 0; i < layoutDesc.Elements.Count; i++)
            {
                if ((layoutDesc.Elements[i].Options & ResourceLayoutElementOptions.DynamicBinding) != 0)
                {
                    ...
                    uint32 desiredOffset = dynamicOffsets[dynamicOffsetIndex];
                    dynamicOffsetIndex += 1;
                    DeviceBufferRange range = Util.GetBufferRange(rs.Resources[i], desiredOffset);

                    if ((range.Offset % requiredAlignment) != 0)
                    {
                        Runtime.FatalError(scope $"The effective offset of the buffer in slot {i} does not meet the alignment requirements of this device. The offset must be a multiple of {requiredAlignment}, but it is {range.Offset}");
                    }
                }
            }
        ...
        public  void SetComputeResourceSet(uint32 slot, ResourceSet rs, uint32 dynamicOffsetsCount, uint32* dynamicOffsets)
        {
            ...
            {
				//Runtime.FatalError(scope $"A compute Pipeline must be active before {nameof(SetComputeResourceSet)} can be called.");
                Runtime.FatalError(scope $"A compute Pipeline must be active before SetComputeResourceSet can be called.");
            }
        ...
        public void ClearColorTarget(uint index, RgbaFloat clearColor)
        {
            ...
            {
                Runtime.FatalError($"Cannot use ClearColorTarget. There is no Framebuffer bound.");
            }
        ...
        internal protected abstract void ClearColorTargetCore(uint32 index, RgbaFloat clearColor);
        ...
        public void ClearDepthStencil(float depth, uint8 stencil)
        {
            ...
            if (_framebuffer == null)
            {
                Runtime.FatalError($"Cannot use ClearDepthStencil. There is no Framebuffer bound.");
            }
        ...
        internal protected abstract void ClearDepthStencilCore(float depth, uint8 stencil);
        ...
        public void SetFullViewports()
        {
            ...
            for (uint32 index = 1; index < _framebuffer.ColorTargets.Length; index++)
            {
        ...
        public abstract void SetViewport(uint32 index, Viewport viewport);
        ...
        internal protected abstract void DrawCore(uint32 vertexCount, uint32 instanceCount, uint32 vertexStart, uint32 instanceStart);
        ...
        public void DrawIndexed(uint32 indexCount, uint32 instanceCount, uint32 indexStart, int32 vertexOffset, uint32 instanceStart)
        {
            ...
            {
                Runtime.FatalError("Drawing with a non-zero base vertex is not supported on this device.");
            }
            if (!_features.DrawBaseInstance && instanceStart != 0)
            {
                Runtime.FatalError("Drawing with a non-zero base instance is not supported on this device.");
            }
#endif
        ...
        internal protected abstract void DrawIndexedCore(uint32 indexCount, uint32 instanceCount, uint32 indexStart, int32 vertexOffset, uint32 instanceStart);
        ...
        private static void ValidateIndirectOffset(uint32 offset)
        {
            if ((offset % 4) != 0)
            {
				//Runtime.FatalError(scope $"{nameof(offset)} must be a multiple of 4.");
                Runtime.FatalError("offset must be a multiple of 4.");
            }
        }
        ...
        private void ValidateDrawIndirectSupport()
        {
            ...
            {
                Runtime.FatalError($"Indirect drawing is not supported by this device.");
            }
        }
        ...
        private static void ValidateIndirectBuffer(DeviceBuffer indirectBuffer)
        {
            ...
            {
				//Runtime.FatalError( scope $"{nameof(indirectBuffer)} parameter must have been created with BufferUsage.IndirectBuffer. Instead, it was {indirectBuffer.Usage}.");
                Runtime.FatalError( scope $"indirectBuffer parameter must have been created with BufferUsage.IndirectBuffer. Instead, it was {indirectBuffer.Usage}.");
            }
        }
        ...
        private static void ValidateIndirectStride(uint32 stride, int32 argumentSize)
        {
            ...
            {
				//Runtime.FatalError(scope $"{nameof(stride)} parameter must be a multiple of 4, and must be larger than the size of the corresponding argument structure.");
                Runtime.FatalError(scope $"stride parameter must be a multiple of 4, and must be larger than the size of the corresponding argument structure.");
            }
        }
        ...
        public void ResolveTexture(Texture source, Texture destination)
        {
            ...
            {
				//Runtime.FatalError(scope $"The {nameof(source)} parameter of {nameof(ResolveTexture)} must be a multisample texture.");
                Runtime.FatalError(scope $"The source parameter of ResolveTexture must be a multisample texture.");
            }
            if (destination.SampleCount != TextureSampleCount.Count1)
            {
				//Runtime.FatalError(scope $"The {nameof(destination)} parameter of {nameof(ResolveTexture)} must be a non-multisample texture. Instead, it is a texture with {FormatHelpers.GetSampleCountUInt32(source.SampleCount)} samples.");
                Runtime.FatalError(scope $"The destination parameter of ResolveTexture must be a non-multisample texture. Instead, it is a texture with {FormatHelpers.GetSampleCountUInt32(source.SampleCount)} samples.");
            }
#endif
        ...
        {
            UpdateBuffer(buffer, bufferOffsetInBytes, &source, (.)sizeof(T));
        }
            ...
            uint32 sizeInBytes)
        {
            ...
            {
                Runtime.FatalError(scope $"The DeviceBuffer's capacity ({buffer.SizeInBytes}) is not large enough to store the amount of data specified ({sizeInBytes}) at the given offset ({bufferOffsetInBytes}).");
            }
            if (sizeInBytes == 0)
            {
                return;
        ...
        internal protected abstract void UpdateBufferCore(
            DeviceBuffer buffer,
            uint32 bufferOffsetInBytes,
            void* source,
            uint32 sizeInBytes);
        ...
        public void CopyTexture(Texture source, Texture destination)
        {
            ...
            {
                Runtime.FatalError("Source and destination Textures are not compatible to be copied.");
            }
#endif
        ...
        public void CopyTexture(Texture source, Texture destination, uint32 mipLevel, uint32 arrayLayer)
        {
            ...
            {
				//Runtime.FatalError(scope $"{nameof(mipLevel)} and {nameof(arrayLayer)} must be less than the given Textures' mip level count and array layer count.");
                Runtime.FatalError(scope $"mipLevel and arrayLayer must be less than the given Textures' mip level count and array layer count.");
            }
#endif
            ...
            uint32 layerCount)
        {
            ...
            {
                Runtime.FatalError($"The given copy region is empty.");
            }
            if (layerCount == 0)
            {
				//Runtime.FatalError(scope $"{nameof(layerCount)} must be greater than 0.");
                Runtime.FatalError(scope $"layerCount must be greater than 0.");
            }
            ...
            {
                Runtime.FatalError($"The given copy region is not valid for the source Texture.");
            }
            ...
            {
                Runtime.FatalError($"The given copy region is not valid for the destination Texture.");
            }
            if (srcMipLevel >= source.MipLevels)
            {
				//Runtime.FatalError(scope $"{nameof(srcMipLevel)} must be less than the number of mip levels in the source Texture.");
                Runtime.FatalError(scope $"srcMipLevel must be less than the number of mip levels in the source Texture.");
            }
            uint32 effectiveSrcArrayLayers = (source.Usage & TextureUsage.Cubemap) != 0
                ? source.ArrayLayers * 6
                : source.ArrayLayers;
            ...
            {
                Runtime.FatalError($"An invalid mip range was given for the source Texture.");
            }
            if (dstMipLevel >= destination.MipLevels)
            {
				//Runtime.FatalError(scope $"{nameof(dstMipLevel)} must be less than the number of mip levels in the destination Texture.");
                Runtime.FatalError(scope $"dstMipLevel must be less than the number of mip levels in the destination Texture.");
            }
            uint32 effectiveDstArrayLayers = (destination.Usage & TextureUsage.Cubemap) != 0
                ? destination.ArrayLayers * 6
                : destination.ArrayLayers;
            ...
            {
                Runtime.FatalError($"An invalid mip range was given for the destination Texture.");
            }
#endif
            CopyTextureCore(
                source,
        ...
        public void GenerateMipmaps(Texture texture)
        {
            ...
            {
				//Runtime.FatalError(scope $"{nameof(GenerateMipmaps)} requires a target Texture with {nameof(TextureUsage)}.{nameof(TextureUsage.GenerateMipmaps)}");
                Runtime.FatalError(scope $"GenerateMipmaps requires a target Texture with TextureUsage.GenerateMipmaps");
            }
        ...
        internal protected abstract void GenerateMipmapsCore(Texture texture);
        ...
        internal protected abstract void PushDebugGroupCore(String name);
        ...
        internal protected abstract void PopDebugGroupCore();
        ...
        internal protected abstract void InsertDebugMarkerCore(String name);
        ...
        private void ValidateIndexBuffer(uint32 indexCount)
        {
            ...
            {
				//Runtime.FatalError(scope $"An index buffer must be bound before {typeof(CommandList).GetName(.. scope .())}.{nameof(DrawIndexed)} can be called.");
                Runtime.FatalError(scope $"An index buffer must be bound before {typeof(CommandList).GetName(.. scope .())}.DrawIndexed can be called.");
            }
        ...
        private void PreDrawValidation()
        {
            ...
            if (_graphicsPipeline == null)
            {
                Runtime.FatalError(scope $"A graphics {typeof(Pipeline).GetName(.. scope .())} must be set in order to issue draw commands.");
            }
            if (_framebuffer == null)
            {
                Runtime.FatalError(scope $"A {typeof(Framebuffer).GetName(.. scope .())} must be set in order to issue draw commands.");
            }
            if (!_graphicsPipeline.GraphicsOutputDescription.Equals(_framebuffer.OutputDescription))
            {
                Runtime.FatalError(scope $"The {typeof(OutputDescription).GetName(.. scope .())} of the current graphics {typeof(Pipeline).GetName(.. scope .())} is not compatible with the current {typeof(Framebuffer).GetName(.. scope .())}.");
            }
#endif
        }
    }
