using Sedulous.Foundation.Utilities;
using System;
namespace Sedulous.Platform;

/// <summary>
/// Represent a control/widget on a WindowSystem.
/// </summary>
public abstract class Surface
{
	/// <summary>
	/// Surface information.
	/// </summary>
	public SurfaceInfo SurfaceInfo;

	/// <summary>
	/// Surface Width.
	/// </summary>
	public uint32 Width;

	/// <summary>
	/// Surface Height.
	/// </summary>
	public uint32 Height;

	/// <summary>
	/// Gets or sets the surface DPI density.
	/// </summary>
	public float DPIDensity { get; protected set; } = 1f;


	/// <summary>
	/// Gets the keyboard events dispatcher associated to this surface.
	/// </summary>
	//public abstract KeyboardDispatcher KeyboardDispatcher { get; }

	/// <summary>
	/// Gets the mouse events dispatcher associated to this surface.
	/// </summary>
	//public abstract MouseDispatcher MouseDispatcher { get; }

	/// <summary>
	/// Gets the touch events dispatcher associated to this surface.
	/// </summary>
	//public abstract PointerDispatcher TouchDispatcher { get; }

	/// <summary>
	/// Occurs when surface size is changed.
	/// </summary>
	public EventAccessor<delegate void(uint32 width, uint32 height)> Resized = new .();

	/// <summary>
	/// Occurs when surface info is changed.
	/// </summary>
	public EventAccessor<delegate void(in SurfaceInfo surfaceInfo)> SurfaceInfoChanged = new .();

	/// <summary>
	/// Occurs when surface is closing
	/// </summary>
	public EventAccessor<delegate void()> Closing = new .();

	/// <summary>
	/// Occurs when surface get focus
	/// </summary>
	public EventAccessor<delegate void()> FocusGained = new .();

	/// <summary>
	/// Occurs when surface lost focus
	/// </summary>
	public EventAccessor<delegate void()> FocusLost = new .();

	/// <summary>
	/// Initializes a new instance of the <see cref="T:Sedulous.Platform.Surface" /> class.
	/// </summary>
	/// <param name="width">surface width.</param>
	/// <param name="height">surface height.</param>
	public this(uint32 width, uint32 height)
	{
		Width = width;
		Height = height;
	}

	public ~this()
	{
		delete Resized;
		delete SurfaceInfoChanged;
		delete Closing;
		delete FocusGained;
		delete FocusLost;


		int32 num = 0;
		while (SurfaceInfo.Handles != null && num < SurfaceInfo.Handles.Count)
		{
			SurfaceInfo.Handles[num] = null;
			num++;
		}

		SurfaceInfo.Dispose();
	}

	/// <summary>
	/// Raise base window closing event.
	/// </summary>
	protected virtual void OnClosing()
	{
		if (this.Closing.[Friend]mEvent.HasListeners)
			this.Closing.[Friend]mEvent();
	}

	/// <summary>
	/// Raise base got focus event.
	/// </summary>
	protected virtual void OnFocusGained()
	{
		if (this.FocusGained.[Friend]mEvent.HasListeners)
			this.FocusGained.[Friend]mEvent();
	}

	/// <summary>
	/// Raise base lost focus event.
	/// </summary>
	protected virtual void OnFocusLost()
	{
		if (this.FocusLost.[Friend]mEvent.HasListeners)
			this.FocusLost.[Friend]mEvent();
	}

	/// <summary>
	/// Raise base size changed event.
	/// </summary>
	protected virtual void OnResized()
	{
		if (this.Resized.[Friend]mEvent.HasListeners)
			this.Resized.[Friend]mEvent(Width, Height);
	}

	/// <summary>
	/// Raise surface info changed.
	/// </summary>
	protected virtual void OnSurfaceInfoChanged()
	{
		if (this.SurfaceInfoChanged.[Friend]mEvent.HasListeners)
			this.SurfaceInfoChanged.[Friend]mEvent(SurfaceInfo);
	}
}