...
namespace Sedulous.RHI.Helpers
{
	public static
	{
		public const int BASE_UPLOAD_BUFFER_SIZE = 65536;
	}
	...
	class DataUploadHelper
	{
		...
		private bool CopyTextureContent(TextureUploadDesc textureUploadDesc, ref uint16 arrayOffset, ref uint16 mipOffset, ref bool isCapacityInsufficient)
		{
			...
			for (; arrayOffset < textureUploadDesc.arraySize; arrayOffset++)
			{
				...
				{
					readonly ref TextureSubresourceUploadDesc subresource = ref textureUploadDesc.subresources[arrayOffset * textureUploadDesc.mipNum + mipOffset];
		...
		private Result DoTransition<T>(T isInitialTransition, in TextureUploadDesc* textureUploadDescs, uint32 textureDataDescNum) where T : const bool
		{
			...
			TextureTransitionBarrierDesc[TEXTURES_PER_PASS] textureTransitions = .();
		...
		private Result DoTransition<T>(T isInitialTransition, in BufferUploadDesc* bufferUploadDescs, uint32 bufferUploadDescNum) where T : const bool
		{
			...
			BufferTransitionBarrierDesc[BUFFERS_PER_PASS] bufferTransitions = .();
		...
		public this(ref Device device, in DeviceAllocator allocator, ref CommandQueue commandQueue)
		{
			...
			m_CommandAllocators = Allocate!<List<CommandAllocator>>(m_Allocator);
			m_CommandBuffers = Allocate!<List<CommandBuffer>>(m_Allocator);
			m_UploadBufferSize = BASE_UPLOAD_BUFFER_SIZE;
		}

		public ~this()
		{
			Deallocate!(m_Allocator, m_CommandBuffers);
