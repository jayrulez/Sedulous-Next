	...
	internal static  class VKSurfaceUtil
	{
		internal static VkSurfaceKHR CreateSurface(VKGraphicsDevice gd, VkInstance instance, SwapchainSource swapchainSource)
		{
			...
			if (doCheck && !gd.HasSurfaceExtension(CommonStrings.VK_KHR_SURFACE_EXTENSION_NAME))
				Runtime.FatalError(scope $"The required instance extension was not available: {CommonStrings.VK_KHR_SURFACE_EXTENSION_NAME}");

			if (Win32SwapchainSource win32Source = swapchainSource as Win32SwapchainSource)
			{
				if (doCheck && !gd.HasSurfaceExtension(CommonStrings.VK_KHR_WIN32_SURFACE_EXTENSION_NAME))
				{
					Runtime.FatalError(scope $"The required instance extension was not available: {CommonStrings.VK_KHR_WIN32_SURFACE_EXTENSION_NAME}");
				}
			...
			switch (swapchainSource)
			{
				/*case XlibSwapchainSource xlibSource:
					...
					{
						Runtime.FatalError(scope $"The required instance extension was not available: {CommonStrings.VK_KHR_XLIB_SURFACE_EXTENSION_NAME}");
					}
				...
				/*case WaylandSwapchainSource waylandSource:
					...
					{
						Runtime.FatalError(scope $"The required instance extension was not available: {CommonStrings.VK_KHR_WAYLAND_SURFACE_EXTENSION_NAME}");
					}
				...
				case AndroidSurfaceSwapchainSource androidSource:
					...
					{
						Runtime.FatalError(scope $"The required instance extension was not available: {CommonStrings.VK_KHR_ANDROID_SURFACE_EXTENSION_NAME}");
					}
				...
				/*case NSWindowSwapchainSource nsWindowSource:
					if (doCheck)
					{
						...
						{
							Runtime.FatalError(scope $"Neither macOS surface extension was available: {CommonStrings.VK_MVK_MACOS_SURFACE_EXTENSION_NAME}, {CommonStrings.VK_EXT_METAL_SURFACE_EXTENSION_NAME}");
						}
					}
				...
				/*case NSViewSwapchainSource nsViewSource:
					if (doCheck)
					{
						...
						{
							Runtime.FatalError($"Neither macOS surface extension was available: " +
			...
			default:
				Runtime.FatalError($"The provided SwapchainSource cannot be used to create a Vulkan surface.");
			}
		}

		private static VkSurfaceKHR CreateWin32(VkInstance instance, Win32SwapchainSource win32Source)
		{
			VkWin32SurfaceCreateInfoKHR surfaceCI = VkWin32SurfaceCreateInfoKHR();
