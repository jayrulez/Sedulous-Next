...
namespace Sedulous.GAL.Vulkan
{
    ...
    internal  class VKCommandList : CommandList
    {
        ...
        private VkRect2D[] _scissorRects = new VkRect2D[0] ~ delete _;

        private VkClearValue[] _clearValues = new VkClearValue[0] ~ delete _;
        private bool[] _validColorClearValues = new bool[0] ~ delete _;
        private VkClearValue? _depthClearValue;
        private readonly List<VKTexture> _preDrawSampledImages = new List<VKTexture>() ~ delete _;
        ...
        private BoundResourceSetInfo[] _currentGraphicsResourceSets = new BoundResourceSetInfo[0] ~ delete _;
        ...
        private BoundResourceSetInfo[] _currentComputeResourceSets = new BoundResourceSetInfo[0] ~ delete _;
        private bool[] _computeResourceSetsChanged;
        private String _name;

        private readonly Monitor _commandBufferListLock = new .() ~ delete _;
        private readonly Queue<VkCommandBuffer> _availableCommandBuffers = new Queue<VkCommandBuffer>() ~ delete _;
        private readonly List<VkCommandBuffer> _submittedCommandBuffers = new List<VkCommandBuffer>() ~ delete _;

        private StagingResourceInfo _currentStagingInfo;
        private readonly Monitor _stagingLock = new .() ~ delete _;
        private readonly Dictionary<VkCommandBuffer, StagingResourceInfo> _submittedStagingInfos = new Dictionary<VkCommandBuffer, StagingResourceInfo>() ~ delete _;
        private readonly List<StagingResourceInfo> _availableStagingInfos = new List<StagingResourceInfo>() ~ delete _;
        private readonly List<VKBuffer> _availableStagingBuffers = new List<VKBuffer>() ~ delete _;
        ...
        public this(VKGraphicsDevice gd, CommandListDescription description)
            : base(description, gd.Features, gd.UniformBufferMinOffsetAlignment, gd.StructuredBufferMinOffsetAlignment)
        {
            _gd = gd;
            VkCommandPoolCreateInfo poolCI = VkCommandPoolCreateInfo();
        ...
        private VkCommandBuffer GetNextCommandBuffer()
        {
            ...
            VkCommandBufferAllocateInfo cbAI = VkCommandBufferAllocateInfo();
        ...
        internal protected override void ClearColorTargetCore(uint32 index, RgbaFloat clearColor)
        {
        ...
        internal protected override void ClearDepthStencilCore(float depth, uint8 stencil)
        {
            VkClearValue clearValue = VkClearValue { depthStencil = VkClearDepthStencilValue(){ depth = depth, stencil = stencil} };
        ...
        internal protected override void DrawCore(uint32 vertexCount, uint32 instanceCount, uint32 vertexStart, uint32 instanceStart)
        {
            PreDrawCommand();
            vkCmdDraw(_cb, vertexCount, instanceCount, vertexStart, instanceStart);
        }

        internal protected override void DrawIndexedCore(uint32 indexCount, uint32 instanceCount, uint32 indexStart, int32 vertexOffset, uint32 instanceStart)
        {
            PreDrawCommand();
        ...
        public override void End()
        {
            if (!_commandBufferBegun)
            {
                Runtime.FatalError("CommandBuffer must have been started before End() may be called.");
            }
        ...
        protected override void SetFramebufferCore(Framebuffer fb)
        {
            ...
            Util.EnsureArrayMinimumSize(ref _scissorRects, Math.Max((uint32)1, (uint32)vkFB.ColorTargets.Length));
            uint32 clearValueCount = (uint32)vkFB.ColorTargets.Length;
            ...
            if (VKSwapchainFramebuffer scFB = fb as VKSwapchainFramebuffer)
            {
        ...
        private void BeginCurrentRenderPass()
        {
            ...
            for (int32 i = 0; i < _currentFramebuffer.ColorTargets.Length; i++)
            {
            ...
            VkRenderPassBeginInfo renderPassBI = VkRenderPassBeginInfo();
            ...
            if (!haveAnyAttachments || !haveAllClearValues)
            {
                ...
                if (haveAnyClearValues)
                {
                    ...
                    for (uint32 i = 0; i < _currentFramebuffer.ColorTargets.Length; i++)
                    {
                        if (_validColorClearValues[i])
                        {
        ...
        internal protected override void SetVertexBufferCore(uint32 index, DeviceBuffer buffer, uint32 offset)
        {
            VKBuffer vkBuffer = Util.AssertSubtype<DeviceBuffer, VKBuffer>(buffer);
            Bulkan.VkBuffer deviceBuffer = vkBuffer.DeviceBuffer;
            uint64 offset64 = offset;
            vkCmdBindVertexBuffers(_cb, index, 1, & deviceBuffer, & offset64);
            _currentStagingInfo.Resources.Add(vkBuffer.RefCount);
        }

        internal protected override void SetIndexBufferCore(DeviceBuffer buffer, IndexFormat format, uint32 offset)
        {
            VKBuffer vkBuffer = Util.AssertSubtype<DeviceBuffer, VKBuffer>(buffer);
        ...
        internal protected override void SetPipelineCore(Pipeline pipeline)
        {
        ...
        public override void SetViewport(uint32 index, Viewport viewport)
        {
            if (index == 0 || _gd.Features.MultipleViewports)
            {
            ...
            uint32 sizeInBytes)
        {
            ...
            vkCmdPipelineBarrier(
                _cb,
                VkPipelineStageFlags.VK_PIPELINE_STAGE_TRANSFER_BIT, VkPipelineStageFlags.VK_PIPELINE_STAGE_VERTEX_INPUT_BIT,
                VkDependencyFlags.None,
                1, & barrier,
                0, null,
                0, null);
            ...
            uint32 layerCount)
        {
            ...
            {
                Bulkan.VkBuffer srcBuffer = srcVkTexture.StagingBuffer;
            ...
            else if (!sourceIsStaging && destIsStaging)
            {
                ...
                Bulkan.VkBuffer dstBuffer = dstVkTexture.StagingBuffer;
        ...
        internal protected override void GenerateMipmapsCore(Texture texture)
        {
            EnsureNoRenderPass();
        ...
        private VKBuffer GetStagingBuffer(uint32 size)
        {
            using (_stagingLock.Enter())
            {
                ...
                if (ret == null)
                {
                    ...
                    ret.Name =  scope $"Staging Buffer (CommandList {_name})"; // todo 
                }
        ...
        internal protected override void PushDebugGroupCore(String name)
        {
            vkCmdDebugMarkerBeginEXTFunction func = _gd.MarkerBegin;
            if (func == null) { return; }

            VkDebugMarkerMarkerInfoEXT markerInfo = VkDebugMarkerMarkerInfoEXT();

            markerInfo.pMarkerName = name.CStr();

            func(_cb, &markerInfo);
        }

        internal protected override void PopDebugGroupCore()
        {
            vkCmdDebugMarkerEndEXTFunction func = _gd.MarkerEnd;
            if (func == null) { return; }

            func(_cb);
        }

        internal protected override void InsertDebugMarkerCore(String name)
        {
            vkCmdDebugMarkerInsertEXTFunction func = _gd.MarkerInsert;
            if (func == null) { return; }
        ...
        private void DisposeCore()
        {
            if (!_destroyed)
            {
				...
				for(var stagingInfo in _availableStagingInfos){
					delete stagingInfo;
				}
            }
        }
        ...
        {
            public List<VKBuffer> BuffersUsed { get; } = new List<VKBuffer>() ~ delete _;
            public HashSet<ResourceRefCount> Resources { get; } = new HashSet<ResourceRefCount>() ~ delete _;
            public void Clear()
            {
                BuffersUsed.Clear();
                Resources.Clear();
