...
namespace Sedulous.GAL.Vulkan
{
    ...
    internal  class VKSwapchainFramebuffer : VKFramebufferBase
    {
        ...
        public override Bulkan.VkFramebuffer CurrentFramebuffer => _scFramebuffers[(int32)_currentImageIndex].CurrentFramebuffer;
        ...
        public override Span<FramebufferAttachment> ColorTargets => _scColorTextures[(int32)_currentImageIndex];
		...
		private List<VKTexture> _referencedTextures = new .() ~ {DeleteContainerAndItems!(_);};
            ...
            VkExtent2D swapchainExtent)
        {
            ...
            result = vkGetSwapchainImagesKHR(_gd.Device, deviceSwapchain, & scImageCount, &_scImages[0]);
            CheckResult(result);
        ...
        private void DestroySwapchainFramebuffers()
        {
            ...
            {
                for (int32 i = 0; i < _scFramebuffers.Count; i++)
                {
                    _scFramebuffers[i]?.Dispose();
					if(_scFramebuffers[i] != null)
					delete _scFramebuffers[i]; // sedulous cleanup
        ...
        private void CreateDepthTexture()
        {
            if (_depthFormat.HasValue)
            {
                ...
                _depthAttachment = FramebufferAttachment(depthTexture, 0);
            }
        }
        ...
        private void CreateFramebuffers()
        {
            ...
            for (uint32 i = 0; i < _scImages.Count; i++)
            {
                ...
                _scColorTextures[i] = new FramebufferAttachment[] ( FramebufferAttachment(colorTex, 0) );
				_referencedTextures.Add(colorTex);
            }
        }
        ...
        protected override void DisposeCore()
        {
            if (!_destroyed)
            {
				...
				DeleteContainerAndItems!(_scColorTextures);

				delete _scFramebuffers;
				delete _scImages;
            }
        }
    }
