        ...
        internal static VkFormat VdToVkPixelFormat(PixelFormat format, bool toDepthFormat = false)
        {
            switch (format)
            {
                ...
                case PixelFormat.R32_Float:
                    return toDepthFormat ? VkFormat.VK_FORMAT_D32_SFLOAT : VkFormat.VK_FORMAT_R32_SFLOAT;
                ...
                default:
                    Runtime.FatalError(scope $"Invalid {typeof(PixelFormat).GetName(.. scope .())}: {format}");
            }
        }
    }
}
