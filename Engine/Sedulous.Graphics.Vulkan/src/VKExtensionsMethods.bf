...
{
	/// <summary>
	/// Extensions methods used to convert valuesfrom Sedulousto DirectX.
	/// </summary>
	public static class VKExtensionsMethods
	{
		...
		{
			VkShaderStageFlags vkShaderStageFlags = VkShaderStageFlags.None;
			if ((stage & ShaderStages.Vertex) != 0)
			{
				vkShaderStageFlags |= VkShaderStageFlags.VK_SHADER_STAGE_VERTEX_BIT;
			}
			if ((stage & ShaderStages.Hull) != 0)
			{
				vkShaderStageFlags |= VkShaderStageFlags.VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT;
			}
			if ((stage & ShaderStages.Domain) != 0)
			{
				vkShaderStageFlags |= VkShaderStageFlags.VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT;
			}
			if ((stage & ShaderStages.Geometry) != 0)
			{
				vkShaderStageFlags |= VkShaderStageFlags.VK_SHADER_STAGE_GEOMETRY_BIT;
			}
			if ((stage & ShaderStages.Pixel) != 0)
			{
				vkShaderStageFlags |= VkShaderStageFlags.VK_SHADER_STAGE_FRAGMENT_BIT;
			}
			if ((stage & ShaderStages.Compute) != 0)
			{
				vkShaderStageFlags |= VkShaderStageFlags.VK_SHADER_STAGE_COMPUTE_BIT;
			}
			if ((stage & ShaderStages.RayGeneration) != 0)
			{
				vkShaderStageFlags |= VkShaderStageFlags.VK_SHADER_STAGE_RAYGEN_BIT_KHR;
			}
			if ((stage & ShaderStages.Miss) != 0)
			{
				vkShaderStageFlags |= VkShaderStageFlags.VK_SHADER_STAGE_MISS_BIT_KHR;
			}
			if ((stage & ShaderStages.ClosestHit) != 0)
			{
				vkShaderStageFlags |= VkShaderStageFlags.VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR;
			}
			if ((stage & ShaderStages.AnyHit) != 0)
			{
				vkShaderStageFlags |= VkShaderStageFlags.VK_SHADER_STAGE_ANY_HIT_BIT_KHR;
			}
			if ((stage & ShaderStages.Intersection) != 0)
			{
				vkShaderStageFlags |= VkShaderStageFlags.VK_SHADER_STAGE_INTERSECTION_BIT_KHR;
			}
		...
		public static VkBuildAccelerationStructureFlagsKHR ToVulkan(this AccelerationStructureFlags flags)
		{
			switch (flags)
			{
			case AccelerationStructureFlags.AllowUpdate: fallthrough;
			case AccelerationStructureFlags.PerformUpdate:
		...
		public  static uint64 GetBufferAddress(this VkBuffer buffer, VkDevice device)
		{
			...
			VkBufferDeviceAddressInfo vkBufferDeviceAddressInfo2 = vkBufferDeviceAddressInfo;
			return VulkanNative.vkGetBufferDeviceAddress(device, &vkBufferDeviceAddressInfo2);
		}
		...
		public  static uint64 GetAccelerationStructureAddress(this VkAccelerationStructureKHR accelerationStructure, VkDevice device)
		{
			...
			VkAccelerationStructureDeviceAddressInfoKHR vkAccelerationStructureDeviceAddressInfoKHR2 = vkAccelerationStructureDeviceAddressInfoKHR;
			return VulkanNative.vkGetAccelerationStructureDeviceAddressKHR(device, &vkAccelerationStructureDeviceAddressInfoKHR2);
		}
	}
}
