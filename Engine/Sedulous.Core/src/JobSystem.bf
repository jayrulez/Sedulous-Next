...
abstract class Job
{
	...
	public virtual void Cancel()
	{
		...
		// All dependents must be cancelled as well
		for (Job job in mDependents)
		{
			job.Cancel();
		}
	}
...
class JobSystem
{
	...
	public this(Engine engine, int workerCount = 0)
	{
		...
		for (int i = 0; i < workerCount; i++)
		{
			...
			worker.OnJobCompleted = new (job, worker) =>
				{
					using (mCompletedJobsMonitor.Enter())
						mCompletedJobs?.Add(job);
				};

			worker.OnJobCanceled = new (job, worker) =>
				{
